[English Version](README.md)

# 迅的像素地牢

迅的像素地牢是一款开源的传统 Roguelike 地牢爬行者，有着随机化的楼层与敌人，与数百件待以收集和使用的物品。基于 [00-Evan](https://shatteredpixel.com/) 的破碎的像素地牢的[源代码](https://github.com/00-Evan/shattered-pixel-dungeon)开发。

## 相对于《破碎的像素地牢》的主要改动:

### 装备：

大量新武器（包括六阶）；

六阶护甲；

武器与护甲各两种新附魔/刻印、一种新诅咒；

一把像原版落石法杖一样，但经过逻辑更改的法杖；

一种可以提升视野半径的新戒指；

一个取代元素之戒的新神器，像药剂中的无序魔典一样；

七个新饰品；

一个用来装食物的新扩展包裹；

### 消耗品：

五种新炸弹；

四种新魔药、六种新秘药；

十一种新法术结晶；

宝石粉末，像戒指中的奥术聚酯一样；

### 冒险：

两个新的英雄：探险家与怨魂！

五个新挑战；

开发者模式；

一些新徽章。

*查看发行文件中的 Changlog.txt 以得知详细改动。*

目前，迅的像素地牢为安卓与桌面平台编译。你可以在这些地方找到游戏的官方发行版：

[![Itch.io](https://shatteredpixel.com/assets/images/badges/itch.png)](https://shattered-pixel.itch.io/shattered-pixel-dungeon)
[![Github 发行版](https://shatteredpixel.com/assets/images/badges/github.png)](https://github.com/X-U-N-123/Xuns-Pixel-Dungeon/releases)

注意**这个仓库不接受 Pull 请求！** 将代码提供在此是希望其他人发现它对于他们自己的项目有用，而非允许社区贡献。任何种类的 Issue 报告（Bug 报告、功能请求之类）都还是很欢迎的。

如果你想搞点代码工作，你可以在 `/docs` 中找到以下指南:
- [为安卓平台编译](docs/getting-started-android.md)
    - **[如果你计划在 Google Play 上分发，务必阅读这篇指南的末尾。](docs/getting-started-android.md#distributing-your-apk)**
- [为桌面端编译](docs/getting-started-desktop.md)
- [在制作自己的改版时推荐进行的改动](docs/recommended-changes.md)