...
public class Badges {
	...
	public static void validateAllBagsBought( Item bag ) {
		...
		if (bag instanceof VelvetPouch) {
			...
			badge = Badge.BAG_BOUGHT_FOOD_POCKET;
		}
		...
		if (badge != null) {
			...
			if (!local.contains( Badge.ALL_BAGS_BOUGHT ) &&
				...
				local.contains( Badge.BAG_BOUGHT_FOOD_POCKET )) {
	...
	static {
		...
		thirdBossSubclassBadges.put(HeroSubClass.SCOUT, Badge.BOSS_SLAIN_3_SCOUT);
		...
		thirdBossSubclassBadges.put(HeroSubClass.PHANTOM, Badge.BOSS_SLAIN_3_PHANTOM);
		...
		thirdBossSubclassBadges.put(HeroSubClass.HYDROLOGIST, Badge.BOSS_SLAIN_3_HYDROLOGIST);
	}
	...
	public static void validateAdventurerUnlock(){
		if (!isUnlocked(Badge.UNLOCK_ADVENTURER)){
