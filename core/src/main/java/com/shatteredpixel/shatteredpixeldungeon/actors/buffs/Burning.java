...
public class Burning extends Buff implements Hero.Doom {
	...
	public boolean act() {
		...
		if (Dungeon.level.map[target.pos] == Terrain.EMBERS && target instanceof Hero && ((Hero) target).hasTalent(Talent.REKINDLED_EMBER)) {
			detach();
			return true;
		}
	...
	public void detach() {
		...
		if (target instanceof Hero && ((Hero) target).heroClass == HeroClass.WRAITH)
			Buff.affect(target, FireImbue.class).set(
