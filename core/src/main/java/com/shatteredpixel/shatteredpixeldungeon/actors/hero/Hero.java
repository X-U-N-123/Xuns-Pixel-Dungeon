...
public class Hero extends Char {
	...
	private boolean getCloser( final int target ) {
		...
		if (step != -1) {
			...
			if (Dungeon.level.pit[step] && !Dungeon.level.solid[step]
					&& (!isFlying() || buff(Levitation.class) != null && buff(Levitation.class).detachesWithinDelay(delay))){
	...
	public void move(int step, boolean travelling) {
		...
		//the Wraith wearing their WraithArmor are considered flying here
		if (!isFlying() && travelling && !(belongings.armor() instanceof WraithArmor)) {
	...
	public void onOperateComplete() {
		...
		if (curAction instanceof HeroAction.Unlock) {
			...
			WornKey.keyRecharge skele = buff(WornKey.keyRecharge.class);
			WornKey.KeyReplacementTracker keyUseTrack = buff(WornKey.KeyReplacementTracker.class);
			...
			if (skele != null && skele.isCursed() && Random.Int(6) != 0){
				...
				if (door == Terrain.LOCKED_DOOR) {
					...
					hasKey = true;
					Level.set(doorCell, Terrain.DOOR);
					GLog.i( Messages.get(WornKey.class, "force_lock"));
				} else if (door == Terrain.CRYSTAL_DOOR) {
