...
public class KindOfCrossbow extends MeleeWeapon {
	...
	public boolean doUnequip(Hero hero, boolean collect, boolean single) {
		if (super.doUnequip(hero, collect, single)){
			if (hero.buff(ChargedShot.class) != null &&
					!(hero.belongings.weapon() instanceof KindOfCrossbow)
					&& !(hero.belongings.secondWep() instanceof KindOfCrossbow)){
	...
	public float accuracyFactor(Char owner, Char target) {
		if (owner.buff(KindOfCrossbow.ChargedShot.class) != null){
			Actor.add(new Actor() {
	...
	public int proc(Char attacker, Char defender, int damage) {
		...
		if (attacker == Dungeon.hero
			...
			WandOfBlastWave.throwChar(defender,
					...
					true,
					this);
			attacker.buff(KindOfCrossbow.ChargedShot.class).detach();
		}
		return dmg;
	}
	...
	public int max(int lvl) {
		return  4*(tier+1) +    //12/20/28 base, down from 15/25/35
				lvl*(tier);     //+2/4/6 per level, down from +3/5/7
	}
	...
	public String abilityInfo() {
		if (levelKnown){
			return Messages.get(this, "ability_desc", name(), 5-(tier/2)+buffedLvl(), 5-(tier/2)+buffedLvl());
		} else {
			return Messages.get(this, "typical_ability_desc", name(), 5-(tier/2), 5-(tier/2));
		}
	}

	@Override
	public String upgradeAbilityStat(int level) {
		return Integer.toString(5-(tier/2) + level);
	}
