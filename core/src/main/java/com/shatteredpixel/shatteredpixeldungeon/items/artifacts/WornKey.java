...
public class WornKey extends Artifact {
    ...
    public void gainExp( int xpGain ){
        ...
        if (exp > 4+level()){
            ...
            Catalog.countUse(WornKey.class);
        }
    ...
    public CellSelector.Listener targeter = new CellSelector.Listener(){
        ...
        public void onSelect(Integer target) {
            ...
            if (target != null && (Dungeon.level.visited[target] || Dungeon.level.mapped[target])){
                ...
                if (target == curUser.pos){
                    GLog.w(Messages.get(WornKey.class, "invalid_target"));
                    return;
                }

                if (Dungeon.level.adjacent(target, curUser.pos)) {
                    if (Dungeon.level.map[target] == Terrain.LOCKED_EXIT){
                        GLog.w(Messages.get(WornKey.class, "wont_open"));
                        return;
                    }
                    if (Dungeon.level.map[target] == Terrain.LOCKED_DOOR){
                        ...
                        if (charge < 1){
                            GLog.i( Messages.get(WornKey.class, "iron_charges") );
                            return;
                        }
                        ...
                        if (charge < 5) {
                            GLog.i(Messages.get(WornKey.class, "crystal_charges"));
                            return;
                        }
                        Sample.INSTANCE.play(Assets.Sounds.UNLOCK);
                        curUser.sprite.operate(target, new Callback() {
                        ...
                        if (charge < 2) {
                            GLog.i(Messages.get(WornKey.class, "lock_charges"));
                            return;
                        }
                        ...
                        if (Actor.findChar(target) != null){
                            ...
                            if (pushCell != -1 && !Char.hasProp(toMove, Char.Property.IMMOVABLE)){
                                ...
                                GLog.w(Messages.get(WornKey.class, "lock_no_space"));
                                return;
                            }
                        }
                        ...
                        if (charge < 2) {
                            GLog.i(Messages.get(WornKey.class, "gold_charges"));
                            return;
                        }
                        Sample.INSTANCE.play(Assets.Sounds.UNLOCK);
                        curUser.sprite.operate(target, new Callback() {
                ...
                if (charge < 2){
                    GLog.i(Messages.get(WornKey.class, "wall_charges"));
                    return;
                }
        ...
        public String prompt() {
            return Messages.get(WornKey.class, "prompt");
        }
    };
    ...
    public static class KeyReplacementTracker extends Buff {
        ...
        public void processExcessKeys(){
            ...
            if (removed){
                ...
                GLog.i(Messages.get(WornKey.class, "discard"));
            }
        }
